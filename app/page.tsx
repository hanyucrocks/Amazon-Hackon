...
export default function Home() {
  ...
  const [product2Price, setProduct2Price] = useState(products[1].price)
  useEffect(() => {
    // Always fetch the current price for product 2 from the API
    const fetchProduct2Price = async () => {
      const res = await fetch('/api/products/2');
      if (res.ok) {
        const data = await res.json();
        if (data.item && data.item.price && data.item.price.N) {
          setProduct2Price(Number(data.item.price.N));
        } else {
          setProduct2Price(products[1].price);
        }
      } else {
        setProduct2Price(products[1].price);
      }
  ...
  // Helper to get dropped price for each product
  function getDroppedPrice(product: typeof products[0]) {
    // You can customize dropped prices per product here
    if (product.id === 1) return 90;
    if (product.id === 2) return 150;
    if (product.id === 3) return 110;
    if (product.id === 4) return 50;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {products.map((product) => (
            <ProductCard
              key={product.id}
              {...product}
              price={product.id === 2 ? product2Price : product.price}
            />
          ))}
        </div>
      </div>

...
async function pushTransactionToServer(transaction: Transaction) {
  try {
    ...
    console.error('Failed to push transaction to server:', error);
  }
}
...
export function BiometricAuth({ onSuccess, onError, onTabChange, amount, productId }: BiometricAuthProps) {
  ...
  // Utility to get all unsynced completed transactions from IndexedDB
  const getUnsyncedCompletedTransactions = async (): Promise<Transaction[]> => {
    return new Promise((resolve, reject) => {
      if (!dbRef.current) return resolve([]);
      const tx = dbRef.current.transaction(['transactions'], 'readonly');
      const store = tx.objectStore('transactions');
      const request = store.getAll();
      request.onsuccess = () => {
        const all = request.result || [];
        const unsynced = all.filter(
          (tx: Transaction) => tx.status === 'completed' && !tx.syncedAt
        );
        resolve(unsynced);
      };
      request.onerror = () => reject(request.error);
  ...
  const syncCompletedTransactions = async () => {
    if (networkStatus !== 'online') return;
    console.log('Syncing completed transactions...');
    const transactionsToSync = await getUnsyncedCompletedTransactions();
    if (transactionsToSync.length === 0) {
      console.log('No completed transactions to sync');
      return;
    }
    try {
      for (const tx of transactionsToSync) {
        const syncedAt = Date.now();
        const updatedTx = { ...tx, syncedAt };
        // Update in IndexedDB
        if (dbRef.current) {
          const txObj = dbRef.current.transaction(['transactions'], 'readwrite');
          const store = txObj.objectStore('transactions');
          await new Promise((resolve, reject) => {
        ...
        setCompletedTransactions(prev =>
          prev.map(t => (t.id === tx.id ? { ...t, syncedAt } : t))
        );
        // Push to DynamoDB
        await pushTransactionToServer(updatedTx);
        // --- Set localStorage flag for product id 2 ONLY after sync ---
        // Try to get product name from URL (VaultX receives ?product=...)
        const urlParams = new URLSearchParams(window.location.search);
        const productName = urlParams.get('product');
        if (productName && productName.toLowerCase().includes('smart watch series 5')) {
          localStorage.setItem('product2_payment_synced', Date.now().toString());
        }
      }
      console.log('Completed transactions synced successfully');
    } catch (error) {
      console.error('Error syncing completed transactions:', error);
    }
  };
  ...
  useEffect(() => {
    const syncAll = async () => {
      if (networkStatus === 'online') {
        ...
        await syncCompletedTransactions();
        loadTransactions();
      }
    };
    syncAll();
  ...
  const handleTransactionSuccess = async () => {
    ...
    // Get productId from URL and userId from localStorage
    const urlParams = new URLSearchParams(window.location.search);
    ...
    const newTransaction: any = {
      ...
      // --- Set localStorage flag for product id 2 if payment is completed and synced ---
      if (networkStatus === 'online') {
        // Try to get product name from URL (VaultX receives ?product=...)
        const urlParams = new URLSearchParams(window.location.search);
        const productName = urlParams.get('product');
        ...
        await pushTransactionToServer(newTransaction);
